...
export default function QuizStep9() {
    ...
    const handleSelect = (value: string) => {
        console.log("Selected Flexibility:", value);
        router.push("/quiz/10"); // Link to next step
    };
        ...
        <div className="w-full animate-fade-in-up">
            ...
            <div className="space-y-3 md:space-y-4 max-w-xl mx-auto">
                <FlexibilityOption
                    label="Muy flexible"
                    emoji="🤸"
                    onClick={() => handleSelect("muy_flexible")}
                />
                <FlexibilityOption
                    label="Bastante flexible"
                    emoji="🧘"
                    onClick={() => handleSelect("bastante_flexible")}
                />
                <FlexibilityOption
                    label="No muy flexible"
                    emoji="🚶"
                    onClick={() => handleSelect("no_muy_flexible")}
                />
                <FlexibilityOption
                    label="No estoy seguro(a)"
                    emoji="🤔"
                    onClick={() => handleSelect("no_seguro")}
                />
            </div>

            <QuizFooter currentStep={9} />
        </div>
    );
}
...
function FlexibilityOption({ label, emoji, onClick }: FlexibilityOptionProps) {
    return (
        <button
            onClick={onClick}
