...
export default function QuizStep10() {
    ...
    const handleSelect = (value: string) => {
        ...
        router.push("/quiz/11"); // Link to next step
    };
    ...
    return (
        <div className="w-full animate-fade-in-up">
            ...
            <h1 className="text-2xl md:text-3xl font-black text-white text-center mb-8 leading-tight">
                ¿Con qué frecuencia entrenaste en los últimos 3 meses?
            </h1>
            ...
            <div className="space-y-3 md:space-y-4 max-w-xl mx-auto">
                <FrequencyOption
                    ...
                    emoji="🔥"
                    onClick={() => handleSelect("casi_todos_dias")}
                />
                <FrequencyOption
                    label="Varias veces por semana"
                    emoji="💪"
                    onClick={() => handleSelect("varias_semana")}
                />
                <FrequencyOption
                    label="Varias veces por mes"
                    emoji="📅"
                    onClick={() => handleSelect("varias_mes")}
                />
                <FrequencyOption
                    label="No entrené"
                    emoji="❌"
                    onClick={() => handleSelect("no_entrene")}
                />
            </div>

            <QuizFooter currentStep={10} />
        </div>
    );
}
...
function FrequencyOption({ label, emoji, onClick }: FrequencyOptionProps) {
    return (
        <button
            onClick={onClick}
