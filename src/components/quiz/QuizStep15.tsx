...
export default function QuizStep15() {
    ...
    const handleSelect = (value: string) => {
        console.log("Selected Typical Day:", value);
        router.push("/quiz/16"); // Link to next step
    };
    ...
    return (
        <div className="w-full animate-fade-in-up">
            ...
            <h1 className="text-2xl md:text-3xl font-black text-white text-center mb-8 leading-tight">
                ¿Cómo describirías tu día típico?
            </h1>
            ...
            <div className="space-y-3 md:space-y-4 max-w-xl mx-auto">
                <ActivityOption
                    ...
                    emoji="🪑"
                    onClick={() => handleSelect("seated")}
                />
                <ActivityOption
                    label="Constantemente de pie"
                    emoji="🧍"
                    onClick={() => handleSelect("standing")}
                />
                <ActivityOption
                    label="Equilibrio entre estar sentado y moverse"
                    emoji="⚖️"
                    onClick={() => handleSelect("balanced")}
                />
                <ActivityOption
                    label="Mi nivel de actividad cambia a diario"
                    emoji="🔄"
                    onClick={() => handleSelect("variable")}
                />
            </div>

            <QuizFooter currentStep={15} />
        </div>
    );
}
...
function ActivityOption({ label, emoji, onClick }: ActivityOptionProps) {
    return (
        <button
            onClick={onClick}
