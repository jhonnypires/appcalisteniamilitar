...
export default function QuizStep13() {
    ...
    const handleSelect = (value: string) => {
        console.log("Selected Desired Frequency:", value);
        router.push("/quiz/14"); // Link to next step
    };
    ...
    return (
        <div className="w-full animate-fade-in-up">
            ...
            <h1 className="text-2xl md:text-3xl font-black text-white text-center mb-8 leading-tight">
                ¿Cuántas veces por semana te gustaría entrenar?
            </h1>
            ...
            <div className="space-y-3 md:space-y-4 max-w-xl mx-auto">
                <FrequencyOption
                    label="1-2 veces"
                    emoji="📅"
                    onClick={() => handleSelect("1_2_times")}
                />
                <FrequencyOption
                    label="3-4 veces"
                    emoji="💪"
                    onClick={() => handleSelect("3_4_times")}
                />
                <FrequencyOption
                    label="5 o más veces"
                    emoji="🔥"
                    onClick={() => handleSelect("5_plus_times")}
                />
            </div>

            <QuizFooter currentStep={13} />
        </div>
    );
}
